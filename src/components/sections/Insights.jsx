        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-12 fade-in" ref={gridRef}>
          ...
          <div className="border border-border-subtle rounded-[20px] overflow-hidden group cursor-pointer hover:border-teal hover:shadow-2xl hover:shadow-teal/10 transition-all duration-300 h-fit">
            <div className="w-full h-[320px] relative overflow-hidden">
              ...
              />
              <span className="absolute bottom-5 left-6 bg-gold text-navy text-[11px] font-bold tracking-[0.1em] uppercase py-1.5 px-3.5 rounded shadow-lg z-10">
            ...
            <div className="p-8 md:p-12">
              <div className="font-dm-mono text-[11px] text-teal mb-4 uppercase tracking-[0.2em] font-semibold">
                {INSIGHTS.featured.date} • {INSIGHTS.featured.readTime} read
              </div>
              <h3 className="font-playfair text-2xl md:text-3xl font-bold text-navy mb-5 leading-tight group-hover:text-teal transition-colors">
              ...
              <a 
                ...
                className="inline-flex items-center gap-2 text-sm font-bold text-navy hover:text-teal transition-all group/link"
              >
                Read Full Article 
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-4">
              {displayedArticles.map((item, i) => (
                <a 
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="border border-border-subtle rounded-2xl p-6 cursor-pointer hover:border-teal hover:translate-x-1 hover:bg-light-bg/50 transition-all group block shadow-sm hover:shadow-md"
                >
                  <div className="text-[10px] font-bold text-teal uppercase tracking-[0.15em] mb-2">{item.tag}</div>
                  <h4 className="font-bold text-[16px] text-navy mb-2.5 group-hover:text-teal transition-colors leading-snug">{item.title}</h4>
                  <div className="text-[12px] text-slate font-medium">{item.date} • {item.readTime} read</div>
                </a>
              ))}
            </div>
            ...
            <div className="mt-4 flex flex-col gap-4">
              <button 
                ...
                className="w-full py-4 border border-navy/10 rounded-xl text-sm font-bold text-navy hover:bg-navy hover:text-white transition-all duration-300 flex items-center justify-center gap-2"
              >
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="w-full py-4 bg-light-bg/50 border border-transparent rounded-xl text-xs font-semibold text-slate hover:text-teal text-center transition-all"
              >
